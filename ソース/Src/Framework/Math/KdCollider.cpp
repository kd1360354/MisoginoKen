...
bool KdModelCollision::Intersects(const DirectX::BoundingOrientedBox& target, const Math::Matrix& world, KdCollider::CollisionResult* pRes)
{
	...
	m_local_box.Transform(myBox, world);

	// target (OBB) と衝突判定
	return target.Intersects(myBox);
	...
	// TODO: 当たり計算は各自必要に応じて拡張して下さい
	//if (!m_enable || !m_shape) { return false; }
	//std::shared_ptr<KdModelData> spModelDate = m_shape->GetData();
	//if (!spModelDate) { return false; }
	//const auto& dataNodes = spModelDate->GetOriginalNodes();
	...
	//{
	//	const auto& dataNode = dataNodes[i];
	//	const auto& workNode = workNodes[i];
	//	if (!dataNode.m_spMesh) continue;

	//	Math::Matrix localWorld = workNode.m_worldTransform * world;

	//	for (const auto& tri : dataNode.m_spMesh->GetTriangles())
	//	{
	//		DirectX::XMVECTOR v0 = XMVector3TransformCoord(tri.p0, localWorld);
	//		DirectX::XMVECTOR v1 = XMVector3TransformCoord(tri.p1, localWorld);
	//		DirectX::XMVECTOR v2 = XMVector3TransformCoord(tri.p2, localWorld);

	//		// 簡易的に「三角形の頂点のどれかがOBBに入ってるか」を判定
	//		if (target.Contains(v0) || target.Contains(v1) || target.Contains(v2))
	//		{
	//			isHit = true;
	...
	//					DirectX::XMVectorAdd(DirectX::XMVectorAdd(v0, v1), v2),
	//					1.0f / 3.0f
	//				);
	//			}
	//			return true; // 最初にヒットしたら返す
	//		}
	//	}
	//}
