#include "KdInput.h"
#include <Xinput.h> // XInputGetState のために必要
#pragma comment(lib, "xinput.lib") // ライブラリのリンク
...
float KdInputManager::GetWheelDelta(std::string_view name) const
{
	float totalWheelDelta = 0.0f;
	...
	//	if (device.second->GetActiveState() == KdInputCollector::ActiveState::Enable)
	//	{
	//		// GetAxisStateで取得し、Y軸成分 (ホイールの回転量) のみを使用
	//		Math::Vector2 nowDeviceAxis = device.second->GetAxisState(name);

	//		// 全てのデバイスからのホイール入力を合計
	//		totalWheelDelta += nowDeviceAxis.y;
	//	}
	//}

	return totalWheelDelta;
}
...
void KdInputAxisForWindowsWheel::Update()
{
	// ホイール入力はY軸にのみ値を反映させる
	m_axis.x = 0.0f;
	m_axis.y = (float)m_wheelDelta;

	// 次のフレームのために回転量をリセット
	// ※ 注意: ホイールのデルタ値は通常フレームごとにリセットされる必要があります。
	m_wheelDelta = 0;
}
// KdInputCollectorForGamePad
// -------------------------------------------------------------
KdInputCollectorForGamePad::KdInputCollectorForGamePad(int controllerIndex)
	: m_controllerIndex(controllerIndex)
{
	// 初期状態を取得 (初回は prevState も currentState も同じで問題ない)
	XInputGetState(m_controllerIndex, &m_currentState);
	m_prevState = m_currentState;
}

void KdInputCollectorForGamePad::Update()
{
	// 1. 前フレームの状態を保存し、最新の入力を取得
	m_prevState = m_currentState;
	DWORD result = XInputGetState(m_controllerIndex, &m_currentState);

	// 2. ベースクラスの PreUpdate を実行
	for (auto& button : m_spButtons) { button.second->PreUpdate(); }
	for (auto& axis : m_spAxes) { axis.second->PreUpdate(); }

	// 3. コントローラーが接続されているか確認
	if (result != ERROR_SUCCESS)
	{
		// 接続されていない場合、すべての入力を「なし」にリセット
	...
	// 4. ボタンと軸の更新
	if (GetActiveState() == ActiveState::Enable)
	{
		// ボタンの更新
		for (auto& button : m_spButtons)
		{
			// GamePad用のボタンクラスにダウンキャストして、状態を渡す
			if (auto spGamePadButton = std::dynamic_pointer_cast<KdInputButtonForGamePad>(button.second))
			{
				spGamePadButton->Update(m_currentState, m_prevState);
			}
		}
		...
		for (auto& axis : m_spAxes)
		{
			// GamePad用の軸クラスにダウンキャストして、状態を渡す
			if (auto spGamePadAxis = std::dynamic_pointer_cast<KdInputAxisForGamePad>(axis.second))
			{
				spGamePadAxis->Update(m_currentState);
			}
		}
	}
	// 5. 無効状態：すべて入力無しの状態にリセット
	else if (GetActiveState() == ActiveState::Disable)
	{
		for (auto& button : m_spButtons) { button.second->NoInput(); }
		for (auto& axis : m_spAxes) { axis.second->NoInput(); }
	}
	// ActiveState::Monitoring の場合は、Update() が呼ばれない（基底クラスのロジックに依存）
}


// KdInputButtonForGamePad
// -------------------------------------------------------------

KdInputButtonForGamePad::KdInputButtonForGamePad(BYTE triggerThreshold, bool isLeft)
	: m_isTrigger(true), m_triggerThreshold(triggerThreshold), m_isLeft(isLeft)
{
	// トリガーがボタンとして機能する場合
}

void KdInputButtonForGamePad::Update(const XINPUT_STATE& currentState, const XINPUT_STATE& prevState)
{
	if (!m_needUpdate) { return; }
	...
	{
		// トリガーの押下判定
		BYTE nowTrigger = m_isLeft ? currentState.Gamepad.bLeftTrigger : currentState.Gamepad.bRightTrigger;
		nowDown = (nowTrigger > m_triggerThreshold);
	}
	else
	{
		// 通常ボタンの押下判定
		nowDown = (currentState.Gamepad.wButtons & m_buttonMask);
	}

	// 状態の更新ロジック (KdInputButtonForWindowsを参考に)
	if (nowDown)
	{
		if (m_state & Hold) {
			m_state &= ~Press;
...
// KdInputAxisForGamePad
// -------------------------------------------------------------

KdInputAxisForGamePad::KdInputAxisForGamePad(StickType type) : m_type(type)
{
	// 初期化処理
}

void KdInputAxisForGamePad::Update(const XINPUT_STATE& currentState)
{
	// X軸とY軸の生の値
	SHORT rawX, rawY;

	if (m_type == StickType::Left)
	{
		rawX = currentState.Gamepad.sThumbLX;
		rawY = currentState.Gamepad.sThumbLY;
		// デッドゾーンは通常 XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE (7849) を使用
		const float DEADZONE = XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
		const float MAX_VALUE = 32767.0f; // SHORTの最大値

		// デッドゾーン処理
		if ((rawX * rawX + rawY * rawY) < (DEADZONE * DEADZONE))
		{
			m_axis = Math::Vector2::Zero;
		}
		else
		{
			// 正規化
			m_axis.x = rawX / MAX_VALUE;
			m_axis.y = rawY / MAX_VALUE;
		}
	}
	else // Right Stick
	{
		rawX = currentState.Gamepad.sThumbRX;
		rawY = currentState.Gamepad.sThumbRY;
		// デッドゾーンは通常 XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE (8689) を使用
