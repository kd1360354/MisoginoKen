...
public:
	...
	virtual void LoadParameters(const nlohmann::json& _j);
	//各種機能
	...
	//Imgui用の関数
	virtual	void SetDebugDisplay(bool show) {};
	...
	// 相手のワールド行列を受け取る関数を追加
	bool Intersects(const KdCollider::BoxInfo& targetBox, const Math::Matrix& targetWorldMatrix, std::list<KdCollider::CollisionResult>* pResults);
