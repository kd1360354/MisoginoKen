...
public:
	...
	{
		int WindEnble = 0;
		Math::Vector3 windOffset = { 0.0f,0.0f,0.0f };
	};
	...
	void SetWindEnable(bool enable) {
		m_cb10_Ambient.Work().WindEnble = enable;
		m_cb10_Ambient.Write();
	}
	void SetWindValue(const Math::Vector3& Value)
	{
		m_cb10_Ambient.Work().windOffset = Value;
		m_cb10_Ambient.Write();
	}
	...
	// 【追加】視錐台を取得する関数
	const DirectX::BoundingFrustum& GetCurrentFrustum() const { return m_currentFrustum; }
...
private:
	...
	//頂点シェーダk
	KdConstantBuffer<cbAmbient> m_cb10_Ambient;
	...
	DirectX::BoundingFrustum m_currentFrustum;
};
