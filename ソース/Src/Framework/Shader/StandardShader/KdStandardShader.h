...
public:
	...
	struct cbObject
	{
		...
		float LimLightIntensity = 1.0f; // 【新規追加】強度係数
		float LimLightPower = 5.0f; // 【新規追加】強度係数

		Math::Vector2 _blank = { 0.0f,0.0f };
	};
	...
	void SetLimLight(const Math::Vector3& color)
	{
		m_cb0_Obj.Work().LimLightColor = color;
		m_dirtyCBObj = true;
	}

	void SetLimLightIntensity(float intensity)
	{
		m_cb0_Obj.Work().LimLightIntensity = intensity;
		m_dirtyCBObj = true;
	}

	void SetLimLightPower(float pow)
	{
...
private:
	...
	//スキンメッシュ有効かどうか
	void SetSkinMeshObj(bool isSkinMeshObj)
	{
