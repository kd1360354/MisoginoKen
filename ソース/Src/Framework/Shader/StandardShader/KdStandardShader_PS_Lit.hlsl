...
float4 main(VSOutput In) : SV_Target0
{
	...
	{
		//ピクセルから見たカメラの方向
		//float3 v = g_CamPos - In.wPos;
		//v = normalize;
		//float LimLightPow = dot(v, wN);

		float LimLightPow = dot(normalize(g_CamPos - In.wPos), wN);
		LimLightPow = 1 - LimLightPow;
		
		outColor += g_LimLightColor * pow(LimLightPow, g_LimLightPower)* g_LimLightIntensity;
	}
