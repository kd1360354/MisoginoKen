...
void KdShaderManager::Init()
{
	...
	//// 頂点シェーダ4
	//m_cb10_Ambient.Create();
	//m_cb10_Ambient.Write();
	//KdDirect3D::Instance().WorkDevContext()->VSSetConstantBuffers(10, 1, m_cb10_Ambient.GetAddress());
	//KdDirect3D::Instance().WorkDevContext()->PSSetConstantBuffers(10, 1, m_cb10_Ambient.GetAddress());
...
void KdShaderManager::UpdateFrustum()
{
	// カメラの定数バッファから情報を取得
	const cbCamera& cb = GetCameraCB();

	// 1. 射影行列からローカル空間の視錐台を作成
	DirectX::BoundingFrustum::CreateFromMatrix(m_currentFrustum, cb.mProj);

	// 2. View行列を逆行列にして「カメラのワールド行列」を作る
	Math::Matrix mCam = cb.mView.Invert();

	// 3. 視錐台をカメラの位置・向きへトランスフォーム
	m_currentFrustum.Transform(m_currentFrustum, mCam);
}
